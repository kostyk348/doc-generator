...
/**
 * Generates high-quality PDF Base64 string for the document using html2canvas & jsPDF
 */
export const generateDocumentPdfBase64 = async (
  data: DocumentData
): Promise<{ base64: string; filename: string } | null> => {
  try {
    const el = document.getElementById('document-a4-sheet');
    if (!el) {
      console.warn('Document preview sheet element #document-a4-sheet not found in DOM.');
      return null;
    }
    ...
    try {
      ...
      console.warn('Canvas export to JPEG failed, trying PNG fallback:', e);
    ...
    console.error('Error rendering PDF for EML attachment:', err);
    return null;
  }
};
